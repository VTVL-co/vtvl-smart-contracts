...
describe("Claim creation", async function () {
  ...
  it("sets the claim", async () => {
    ...
    // Last field is fractional amount
    [
      startTimestamp,
      endTimestamp,
      cliffReleaseTimestamp,
      ...
      // @ts-ignore
      const returnedValue = returnedClaimInfo?.[v];
      expect(returnedValue).to.be.equal(
        k,
        `New claim not properly set. Error on ${k}. Received ${returnedValue}, expected ${k}.`
      );
    });
  });
  ...
  it("createClaim mint nft", async () => {
    ...
    const balance = await vestingContract.balanceOf(recipientAddress);
...
describe("Withdraw", async () => {
  ...
  it("allows withdrawal up to the allowance and fails after the allowance is spent", async () => {
    ...
    await expect(vestingContract.withdraw(1)).to.be.revertedWith(
      "NOTHING_TO_WITHDRAW"
    );
    ...
    (await vestingContract.withdraw(1)).wait();
    ...
    await (await vestingContract.withdraw(1)).wait();
    ...
    expect(balanceAfterSecondWithdraw).to.be.equal(
      balanceAfterFirstWithdraw.add(linearVestAmount * 0.1)
    );
  });
  it("disallows withdrawal for no nft owner", async () => {
    const { vestingContract } = await createPrefundedVestingContract({
      tokenName,
      tokenSymbol,
      initialSupplyTokens,
    ...
    await expect(
      vestingContract.connect(owner2).withdraw(1)
    ).to.be.revertedWith("NO_NFT_OWNER");
  });
  it("disallows withdrawal for an user with consumed claim", async () => {
    ...
    await (await vestingContract.connect(owner2).withdraw(1)).wait();
    await expect(
      vestingContract.connect(owner2).withdraw(1)
  ...
  it("disallows withdrawal for an user with revoked claim", async () => {
    ...
    await (await vestingContract.revokeClaim(1)).wait();
    await expect(
      vestingContract.connect(owner2).withdraw(1)
...
describe("Revoke Claim", async () => {
  ...
  it("fails to revoke an already withdrawn claim", async () => {
    ...
    await expect(vestingContract.revokeClaim(1)).to.be.revertedWith(
      "NO_UNVESTED_AMOUNT"
    );
  });
  ...
  it("takes revocation into account while calculating the vested amount", async () => {
    ...
    expect(await vestingContract.finalVestedAmount(1)).to.be.equal(0);
  });
...
describe("Vested amount", async () => {
  ...
  it("calculates the vested amount before the cliff time to be 0", async () => {
    expect(
      await vestingContract.vestedAmount(1, cliffReleaseTimestamp.sub(1))
    ).to.be.equal(0);
  });
  it("calculates the vested amount at the cliff time to be equal cliff amount", async () => {
    ...
    expect(
      await vestingContract.vestedAmount(1, cliffReleaseTimestamp)
  ...
  it("correctly calculates the vested amount after the cliff time, but before the linear start time", async () => {
    expect(
      await vestingContract.vestedAmount(1, cliffReleaseTimestamp.add(1))
    ).to.be.equal(cliffAmount);
    expect(
      await vestingContract.vestedAmount(1, startTimestamp.sub(1))
    ).to.be.equal(cliffAmount);
  });
  it("vests correctly if cliff and linear vesting begin are at the same time", async () => {
    ...
    expect(await vestingContract.vestedAmount(1, startTimestamp)).to.be.equal(
      cliffAmount
    );
    // Half the interval past, we've vested the cliff and half the linear amount
    expect(
      await vestingContract.vestedAmount(1, startTimestamp.add(500))
  ...
  it("calculates the vested amount at the end of the linear interval to be the full amount allocated", async () => {
    ...
    expect(await vestingContract.vestedAmount(1, endTimestamp)).to.be.equal(
      cliffAmount.add(linearVestAmount)
    );
  });
  it("doesn't vest further after the end of the linear interval", async () => {
    ...
    expect(
      await vestingContract.vestedAmount(1, endTimestamp.add(100000000))
  ...
  it("calculates the finalVestedAmount to be equal the total amount to be vested", async () => {
    expect(await vestingContract.finalVestedAmount(1)).to.be.equal(
      cliffAmount.add(linearVestAmount)
    );
  });
  it("takes the release interval into account", async () => {
    ...
    const vestedAtStart = await vestingContract.vestedAmount(1, startTimestamp);
...
describe("Claimable amount", async () => {
  ...
  it(`calculates the claimable amount to be equal to the vested amount if we have no withdrawals`, async () => {
    ...
    for (let ts = startTimestamp; ts <= endTimestamp; ts += 100) {
      ...
      const vestAmt = await vestingContract.vestedAmount(1, ts);
      const claimableAmount = await vestingContract.claimableAmount(1);
  ...
  it("takes withdrawals into account when calculating the claimable amount", async () => {
    ...
    const tx = await vestingContract.connect(owner2).withdraw(1);
    ...
    expect(await vestingContract.claimableAmount(1)).to.be.equal(0);
    ...
    const vestAmtEnd = await vestingContract.finalVestedAmount(1);
    const expectedClaimable = vestAmtEnd.sub(amtFirstWithdraw);
    expect(await vestingContract.claimableAmount(1)).to.be.equal(
      expectedClaimable
    );
  });

  it("takes withdrawals into account with fractional Id when calculating the claimable amount", async () => {
    const startTimestamp = (await getLastBlockTs()) + 100;
    const endTimestamp = startTimestamp + 1000;
    const { vestingContract } = await createPrefundedVestingContract({
      tokenName,
  ...
  it("claimable after transfer fractional NFT", async () => {
    const startTimestamp = (await getLastBlockTs()) + 100;
    const endTimestamp = startTimestamp + 1000;
    const { vestingContract } = await createPrefundedVestingContract({
      tokenName,
    ...
    await vestingContract.connect(owner2).withdraw(1);

    // transfer fractional NFT
    await vestingContract
      .connect(owner2)
      .transferFrom(owner2.address, owner3.address, 1);

    await ethers.provider.send("evm_mine", [startTimestamp + 1000]);
    const claimableAmount = await vestingContract.claimableAmount(1);

    expect(claimableAmount.toNumber()).to.be.lessThan(
      await (await vestingContract.claimableAmount(2)).toNumber()
    );

    const tx = await vestingContract.connect(owner3).withdraw(1);
    const withdrawAmount = (await tx.wait()).events?.[1]?.args
      ?._withdrawalAmount;

    expect(withdrawAmount).to.be.equal(claimableAmount);
  });
});
describe("Admin withdrawal", async () => {
  it("allows the admin to withdraw the allocated amount", async () => {
    ...
    expect(await tokenContract.balanceOf(owner.address)).to.be.equal(
      firstWithdrawalAmt
    );
    const remainingBalance = await tokenContract.balanceOf(
      vestingContract.address
    );
    ...
    await expect(
      vestingContract.withdrawAdmin(remainingBalance)
  ...
  it("reverts on attempt to withdraw more than available", async () => {
    ...
    await expect(
      vestingContract.withdrawAdmin(initialBalance.add(1))
    ).to.be.revertedWith("INSUFFICIENT_BALANCE");
  });
...
describe("Other token admin withdrawal", async () => {
  ...
  const createOtherToken = async (vestingContractAddress: string | null) => {
    ...
    const otherTokenContract = await tokenContractFactory
      .connect(otherUser)
      .deploy(tokenName, tokenSymbol, otherTokenAmount);
  ...
  it("allows the admin to withdraw the other token, if present", async () => {
    ...
    expect(myBalance).to.be.equal(
      otherTokenAmount,
      `We didn't get the full amount of tokens (${otherTokenAmount})`
    );
  });
  it("fails on other token withdraw, if token with zero balance", async () => {
    ...
    expect(
      vestingContract.withdrawOtherToken(otherTokenContract.address)
    ).to.be.revertedWith("INSUFFICIENT_BALANCE");
  });
  it("fails if called with the main token", async () => {
    ...
    expect(
      vestingContract.withdrawOtherToken(tokenContract.address)
...
describe("Long vest fail", async () => {
  ...
  it("half term works", async () => {
    expect(await vestingContract.vestedAmount(1, midTimestamp)).to.be.equal(
